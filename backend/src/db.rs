...
impl ListDatabase {
    ...
    pub async fn fetch_list(&self, list: i32) -> Result<List, ListError> {
        match sqlx::query_as!(List, "SELECT * FROM lists WHERE id = $1 ORDER BY id", list)
            .fetch_one(&self.pool)
            .await
        {
            Ok(res) => Ok(res),
    ...
    pub async fn fetch_items(&self, list: i32) -> Result<Vec<Item>, ListError> {
        let res = sqlx::query!("SELECT FROM lists WHERE id = $1", list)
            .execute(&self.pool)
            .await?;
        (res.rows_affected() > 0)
            .then_some(())
    ...
    pub async fn fetch_item(&self, item: i32) -> Result<Item, ListError> {
        match sqlx::query_as!(Item, "SELECT * FROM items WHERE id = $1 ORDER BY id", item)
            .fetch_one(&self.pool)
            .await
        {
            Ok(res) => Ok(res),
        ...
        .await
        {
            ...
            Err(sqlx::Error::Database(err))
                if err
                    ...
                    .unwrap_or_default() =>
            {
                Err(ListError::DuplicateListName(name.into()))
            }
    ...
    pub async fn remove_list(&self, list: i32) -> Result<List, ListError> {
        match sqlx::query_as!(List, "DELETE FROM lists WHERE id = $1 RETURNING *", list)
            .fetch_one(&self.pool)
            .await
        {
            Ok(list) => Ok(list),
    ...
    pub async fn rename_list(&self, list: i32, new_name: &str) -> Result<List, ListError> {
        match sqlx::query_as!(
            List,
        ...
        .await
        {
            ...
            Err(sqlx::Error::Database(err))
                if err
                    ...
                    .unwrap_or_default() =>
            {
                Err(ListError::DuplicateListName(new_name.into()))
            }
            Err(e) => Err(ListError::SqlxError(e)),
        }
    }
    ...
    pub async fn create_item(&self, list: i32, content: &str) -> Result<Item, ListError> {
        match sqlx::query_as!(
            Item,
            "INSERT INTO items (list_id, content) VALUES ($1, $2) RETURNING *",
            list,
            content
        )
        .fetch_one(&self.pool)
    ...
    pub async fn remove_item(&self, item: i32) -> Result<Item, ListError> {
        match sqlx::query_as!(Item, "DELETE FROM items WHERE id = $1 RETURNING *", item)
            .fetch_one(&self.pool)
            .await
        {
            Ok(item) => Ok(item),
    ...
    pub async fn edit_item(&self, item: i32, new_content: &str) -> Result<Item, ListError> {
        match sqlx::query_as!(
            Item,
